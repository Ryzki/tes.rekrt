	...
	<nav class="navbar navbar-expand-lg fixed-bottom navbar-light bg-white shadow">
		<div class="container">
			<ul class="navbar nav ms-auto">
				<li class="nav-item">
					<span id="answered">0</span>/<span id="total">{{ $jumlah_soal }}</span> Soal Terjawab
				</li>
    ...
    $(document).ready(function(){
        ...
        function nextId(num){
            $.ajax({
                ...
                jawabA = data.quest.jawaba
                jawabB = data.quest.jawabb
                items = num;
                console.log(data)
                $('.num').text('Soal '+num)

                labelA = $('<p><label id="id_work_days"><input type="radio" class="radioClass'+num+'" id="radioA'+num+'" name="mbti['+num+']" value="A"><span>'+jawabA[num-1]+'</span></label></p>')
                labelB = $('<p><label id="id_work_days"><input type="radio" class="radioClass'+num+'" id="radioB'+num+'" name="mbti['+num+']" value="B"><span>'+jawabB[num-1]+'</span></label></p>')
                var div_all = $('<div ></div>');
